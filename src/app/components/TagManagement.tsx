...
const TagManagement = () => {
  const { media, tags, setTags } = useWatershed();
  ...
  const colorPalette = [
    ...
    '#84CC16', '#F43F5E', '#8B5CF6', '#0EA5E9', '#F59E0B'
  ];
  ...
  const sortedTags = [...filteredTags].sort((a, b) => {
    const usageA = tagStats.get(a.name) || 0;
    const usageB = tagStats.get(b.name) || 0;
  ...
  const handleUpdate = (id: string) => {
    ...
    // Note: If tag name changed, you may need to update media items
    if (oldTag && oldTag.name !== formData.name) {
      console.log(`Tag renamed from "${oldTag.name}" to "${formData.name}"`);
      // Implement media update logic here if needed
    }
  ...
  const handleDelete = (id: string) => {
    ...
    const usageCount = tagStats.get(tag?.name || '') || 0;

    const confirmMessage = usageCount > 0
      ? `This tag is used by ${usageCount} media item(s). Deleting it will remove it from all media. Are you sure you want to delete "${tag?.name}"?`
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-purple-50 to-fuchsia-50">
      <div className="container mx-auto px-6 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-purple-200">
              ...
              <div className="text-3xl font-bold text-slate-900">
                {Array.from(tagStats.keys()).length}
              </div>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-sm border border-purple-200">
              <div className="flex items-center gap-3 mb-2">
              ...
              <div className="text-3xl font-bold text-slate-900">
                {Array.from(tagStats.values()).reduce((a, b) => a + b, 0)}
              </div>
            </div>
            <div className="bg-white rounded-xl p-6 shadow-sm border border-purple-200">
              <div className="flex items-center gap-3 mb-2">
              ...
              <div className="text-3xl font-bold text-slate-900">
                {tags.length > 0
                  ? (Array.from(tagStats.values()).reduce((a, b) => a + b, 0) / tags.length).toFixed(1)
                  : 0}
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {sortedTags.map((tag) => {
            const usage = tagStats.get(tag.name) || 0;
            return (
              <div
                key={tag.id}
